...
class LIGOSensorSimulator:
    def __init__(self, root):
        ...
        self.address = 0x01
        self.temperature = 25
        self.level = 0  # Start at 0 (empty)
        self.frequency = 2513  # Max frequency when empty
        ...
        # Limits
        self.MAX_LEVEL = 7500  # 0x1D4C
        self.MAX_FREQUENCY = 2513  # 0x62E1
        self.MIN_FREQUENCY = 0  # Frequency at max level
    ...
    def frequency_from_level(self, level):
        """
        Calculate frequency based on level.
        Inverse relationship: level 0 -> max frequency, level MAX_LEVEL -> min frequency (0)
        """
        if self.MAX_LEVEL == 0:
            return self.MIN_FREQUENCY
        # Linear interpolation: frequency = MAX_FREQUENCY - (level / MAX_LEVEL) * MAX_FREQUENCY
        frequency = self.MAX_FREQUENCY - (level / self.MAX_LEVEL) * self.MAX_FREQUENCY
        return max(self.MIN_FREQUENCY, min(self.MAX_FREQUENCY, int(round(frequency))))
    
    def update_frequency_from_level(self):
        """Update frequency based on current level"""
        self.frequency = self.frequency_from_level(self.level)
        self.freq_var.set(str(self.frequency))
        
    def level_from_frequency(self, frequency):
        """
        Calculate level based on frequency.
        Inverse relationship: max frequency -> level 0, min frequency -> level MAX_LEVEL
        """
        if self.MAX_FREQUENCY == self.MIN_FREQUENCY:
            return 0
        # Linear interpolation: level = MAX_LEVEL * (1 - (frequency - MIN_FREQUENCY) / (MAX_FREQUENCY - MIN_FREQUENCY))
        level = self.MAX_LEVEL * (1 - (frequency - self.MIN_FREQUENCY) / (self.MAX_FREQUENCY - self.MIN_FREQUENCY))
        return max(0, min(self.MAX_LEVEL, int(round(level))))
    ...
    def setup_gui(self):
        ...
        # Level slider (0 to MAX_LEVEL)
        level_frame = ttk.Frame(data_frame)
        level_frame.grid(row=1, column=0, columnspan=7, sticky=(tk.W, tk.E), pady=5)
        
        ttk.Label(level_frame, text=f"Level (0-{self.MAX_LEVEL}):").grid(row=0, column=0, sticky=tk.W, padx=5)
        
        self.level_var = tk.StringVar(value="0")
        self.level_slider = ttk.Scale(level_frame, from_=0, to=self.MAX_LEVEL, orient=tk.HORIZONTAL, 
                                     length=400)
        self.level_slider.set(0)
        ...
        # Info label for level-frequency relationship
        ttk.Label(level_frame, text=f"(Empty: {self.MAX_FREQUENCY}Hz, Full: {self.MIN_FREQUENCY}Hz)", 
                 foreground="gray").grid(row=0, column=3, padx=10)
        
        # Frequency (display only, updated automatically from level)
        freq_frame = ttk.Frame(data_frame)
        freq_frame.grid(row=2, column=0, columnspan=7, sticky=(tk.W, tk.E), pady=5)
        
        ttk.Label(freq_frame, text=f"Frequency (0-{self.MAX_FREQUENCY} Hz):").grid(row=0, column=0, sticky=tk.W, padx=5)
        
        self.freq_var = tk.StringVar(value=str(self.MAX_FREQUENCY))
        self.freq_entry = ttk.Entry(freq_frame, textvariable=self.freq_var, width=10, state="readonly")
        ...
        self.current_values_label = ttk.Label(values_frame, 
            text=f"Current Values: Temp=25°C, Level=0 (Empty), Freq={self.MAX_FREQUENCY}Hz", 
            font=('Arial', 10, 'bold'))
    ...
    def update_level_from_slider(self, value):
        """Update level from slider movement - automatically updates frequency"""
        if self.initializing:
            return
        try:
            self.level = int(float(value))
            self.level_var.set(str(self.level))
            # Update frequency based on new level
            self.update_frequency_from_level()
            self.update_current_values_display()
        except ValueError:
            pass
    ...
    def update_level_from_entry(self, event=None):
        ...
        try:
            ...
            self.level = max(0, min(self.MAX_LEVEL, value))
            self.level_var.set(str(self.level))
    ...
    def update_frequency_from_entry(self, event=None):
        """Update frequency from entry field - calculates corresponding level"""
        try:
            value = int(self.freq_var.get())
            frequency = max(self.MIN_FREQUENCY, min(self.MAX_FREQUENCY, value))
            # Calculate level from frequency
            self.level = self.level_from_frequency(frequency)
            ...
            self.freq_var.set(str(self.frequency))
            self.update_current_values_display()
            self.log_message(f"Frequency set to {frequency}Hz, Level calculated as {self.level}")
        except ValueError:
            self.freq_var.set(str(self.frequency))
    ...
    def update_current_values_display(self):
        ...
        if hasattr(self, 'current_values_label'):
            level_status = "Empty" if self.level == 0 else "Full" if self.level >= self.MAX_LEVEL else f"{self.level}"
            self.current_values_label.config(
                text=f"Current Values: Temp={self.temperature}°C, Level={self.level} ({level_status}), Freq={self.frequency}Hz"
            )
    ...
    def update_data(self):
        try:
            ...
            # Level is already constrained by slider max
            ...
            # Update frequency based on level
            self.update_frequency_from_level()
            ...
            self.log_message(f"Sensor data updated: Level={self.level}, Freq={self.frequency}Hz")
        except ValueError:
    ...
    def random_data(self):
        ...
        # Frequency will be auto-updated from level
        self.update_frequency_from_level()
    ...
    def create_response_06(self):
        ...
        # Level (16-bit, little endian) - max 7500 (0x1D4C)
        response.append(self.level & 0xFF)
        response.append((self.level >> 8) & 0xFF)
        
        # Frequency (16-bit, little endian) - max 2513 (0x62E1)
    ...
    def create_ascii_data(self):
        """Create ASCII data string with hex values"""
        # Format: F=XXXXt=XXN=XXXX.0
